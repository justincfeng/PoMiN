...
class Pomin
{
    ...
    private:
        ...
        double G = 1.0;                        // Newton's Constant

        int hamiltonian_to_use = 123; //

        /* Hamiltonian variables.  These are computed by calcHamiltonian() */
        double H[3];

        /* Returns 0, 1.0, or -1.0 */
        double Sign(double x);


        /* PHI variables.  These are computed by calcPhi().
         * These were kept as 1- and 2-dim C-style arrays so that the formula
         * could be easily copied over from the C code */
        ...
        double smallesta, smallestb;    // particle numbers (a and b) for the closest pair
};