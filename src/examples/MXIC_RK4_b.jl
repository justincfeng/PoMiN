...
function main()
    ...
    nn_values = 1:50:250  # Range from 1 to 250 in steps of 50
    ...
    for nn in nn_values
        ...
        dx = 100*b  # Optimal separation (not too large, not too small)
        
        println("  Impact parameter b = $(round(b, sigdigits=6))")
        println("  Initial separation dx = $(round(dx, sigdigits=6))")
        
        # Set up scattering system (testing with Double64 like MXIC.jl)
        system = pomin.setup_scattering(m1, m2, p, b, dx, tpfl=Double64)
        
        # Calculate integration time using MXIC.jl approach (relativistic velocity)
        ...
        δ_initial = BigFloat(0.001)          # Initial timestep (reduced for higher accuracy)
        courant = BigFloat(0.001)          # Courant number (reduced further for maximum accuracy)
