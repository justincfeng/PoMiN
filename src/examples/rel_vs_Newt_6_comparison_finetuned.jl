...
qalphapc = tpfl.([ 0.95845, 
                  -0.93402, 
                  -0.01601])         # in pc

valpha = tpfl.([-29.291, 1.710, 13.589])  # in km/s

# in km/s
palpha = tpfl.([-6.363558578130740E-05, 
                 1.508126516235040E-04, 
                 1.475021586009610E-04])          # in geometric units

# Alpha Centauri barycentric velocity at J2000.0 epoch (from proper motion + radial velocity)
vAlphaCen = tpfl.([-21.600000, -23.271732, 3.048579])  # km/s
vAlphaCen_c = vAlphaCen * 1000 / cMKS  # convert to units of c
# Alternative: vAlphaCen_c = tpfl.([-7.20498446e-05, -7.76261409e-05, 1.01689635e-05])
...
pEarth = tpfl.([-2.9839042080E-10, 
                -5.0388889700E-11, 
                -2.1846049145E-11])       # geometric units

# Earth barycentric velocity at J2000.0 epoch (from astropy)
vEarth = tpfl.([-2.9805566714E+01,
                -5.0332360186E+00,
                -2.1821540835E+00])                # in km/s

vEarth_c = vEarth * 1000 / cMKS  # convert to units of c
