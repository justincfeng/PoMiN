...
# Solve
solN        = pomin.solve(PintN, params; testparticles=PtestN)
...
zendNint    = zendN[1:2*nInt]
zendNtest   = zendN[2*nInt+1:end]

# Proxima
qProxN      = zendNint[1:3]
pProxN      = zendNint[nInt+1:nInt+3]
...
# Separate interacting and test particle phase space vectors
zendRint    = zendR[1:2*nInt]
zendRtest   = zendR[2*nInt+1:end]

# Proxima
qProxR      = zendRint[1:3]
pProxR      = zendRint[nInt+1:nInt+3]
...
function missfuncconstructor(Pint,Ptest0,params,bv,tcl;Newt=false)
    ...
    if Newt
        return function(v)
            ...
            q_spacecraft  = Zend_modified[2*nInt+1:2*nInt+3]
...
J_init_N = ForwardDiff.jacobian(fmN, vcorrN)

println("Broyden iterations (Newtonian)...")
v_finetuned_N = bsolve(fmN, J_init_N, fmN(vcorrN), vcorrN, 10)
...
zendNint_ft = Zend_ft_N[1:2*nInt]
zendNtest_ft = Zend_ft_N[2*nInt+1:end]
...
J_init_R = ForwardDiff.jacobian(fmR, vcorrR)

println("Broyden iterations (Relativistic)...")
v_finetuned_R = bsolve(fmR, J_init_R, fmR(vcorrR), vcorrR, 10)
...
# Extract final positions
zendRint_ft = Zend_ft_R[1:2*nInt]
zendRtest_ft = Zend_ft_R[2*nInt+1:end]
...
println("  Total:       $(Printf.@sprintf("%.6e", Δq_total_mag/AU)) AU  ($(Printf.@sprintf("%.6e", Δq_total_mag/km)) km)")
Δq_parallel = Δq_total .* (v_finetuned_R ./ norm(v_finetuned_R))
Δq_parallel_mag = norm(Δq_parallel)
println("  Parallel:    $(Printf.@sprintf("%.6e", Δq_parallel_mag/AU)) AU  ($(Printf.@sprintf("%.6e", Δq_parallel_mag/km)) km)")
Δq_transverse = Δq_total - Δq_parallel
Δq_transverse_mag = norm(Δq_transverse)
println("  Transverse:  $(Printf.@sprintf("%.6e", Δq_transverse_mag/AU)) AU  ($(Printf.@sprintf("%.6e", Δq_transverse_mag/km)) km)")
