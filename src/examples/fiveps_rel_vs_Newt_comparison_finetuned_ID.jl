...
#   Used Broyden-optimized initial data
#
#-----------------------------------------------------------------------
...
γchip_relN_FT = one(tpfl)/sqrt(one(tpfl)-(norm(Vchip_newt_FT))^2)
pchip_relN_FT = (mchip*γchip_relN_FT) .* Vchip_newt_FT
...
Pchip_NID = pomin.setup_single_particle(mchip, qchip, pchip_relN_FT, tpfl)
...
dmissN = closest_approach(qScN, qtarN, vScN, vtarN)

# Final target position minus initial spacecraft position
ΔxN     = qtarN - qchip

#println("Newtonian miss: ", qmissN)
println("Newtonian miss distance: ", dmissN, " (", dmissN/AU, " AU, ", dmissN*Msol2m/1000, " km)")
...
dmissR = closest_approach(qScR, qtarR, vScR, vtarR)

# Final target position minus initial spacecraft position
ΔxR     = qtarR - qchip

#println("Relativistic miss: ", qmissR)
println("Relativistic miss distance: ", dmissR, " (", dmissR/AU, " AU, ", dmissR*Msol2m/1000, " km)")
println("Relativistic final target position minus initial spacecraft position: ", ΔxR)

#-----------------------------------------------------------------------
#   RELATIVISTIC CASE WITH NEWTONIAN INITIAL DATA
#-----------------------------------------------------------------------

# Solve
solRNID = pomin.solve(Pint, params; testparticles=PtestNID)

# Final positions and momenta
zendRNID = solRNID(tcl)

# Masses
mProxRNID   = Pint.m[1]
mScRNID     = PtestNID.m[1]

# Separate interacting and test particle phase space vectors
zendRNIDint    = zendRNID[1:6*nInt]
zendRNIDtest   = zendRNID[6*nInt+1:end]

# Proxima
qProxRNID      = zendRNIDint[1:3]
pProxRNID      = zendRNIDint[3*nInt+1:3*nInt+3]
vProxRNID      = γV2v(pProxRNID ./ mProxRNID)

# Target
qtarRNID       = qProxRNID + bv
vtarRNID       = vProxRNID

# Starchip
qScRNID        = zendRNIDtest[1:3]
pScRNID        = zendRNIDtest[4:6]
vScRNID        = γV2v(pScRNID ./ mScRNID)

# Miss distance
qmissRNID      = qtarRNID - qScRNID
#dmissR      = norm(qmissR)

dmissRNID = closest_approach(qScRNID, qtarRNID, vScRNID, vtarRNID)

# Final target position minus initial spacecraft position
ΔxRNID     = qtarRNID - qchip

println("Relativistic miss: ", qmissRNID)
println("Relativistic miss distance: ", dmissRNID, " (", dmissRNID/AU, " AU, ", dmissRNID*Msol2m/1000, " km)")
println("Relativistic final target position minus initial spacecraft position: ", ΔxRNID)

#-----------------------------------------------------------------------
#   SAVE FINAL TARGET POSITIONS TO FILE
#-----------------------------------------------------------------------

println("Saving final target positions to Final_Prox_minus_Spacecraft.txt...")

open("Final_Prox_minus_Spacecraft.txt", "w") do file
    ...
    println(file, "")
    println(file, "# Relativistic case with Newtonian fine-tuned initial data:")
    println(file, "qtarRNID = [tpfl(\"$(qtarRNID[1])\"),")
    println(file, "          tpfl(\"$(qtarRNID[2])\"),")
    println(file, "          tpfl(\"$(qtarRNID[3])\")]  # Target position")
    println(file, "")
    println(file, "qScRNID = [tpfl(\"$(qScRNID[1])\"),")
    println(file, "         tpfl(\"$(qScRNID[2])\"),")
    println(file, "         tpfl(\"$(qScRNID[3])\")]   # Spacecraft position")
    println(file, "")
    println(file, "# Miss distances")
    println(file, "dmissN = tpfl(\"$(dmissN)\")  # Newtonian miss distance ($(dmissN/AU) AU, $(dmissN*Msol2m/1000) km)")
    println(file, "dmissR = tpfl(\"$(dmissR)\")  # Relativistic miss distance ($(dmissR/AU) AU, $(dmissR*Msol2m/1000) km)")
    println(file, "dmissRNID = tpfl(\"$(dmissRNID)\")  # Relativistic miss distance with Newtonian fine-tuned initial data ($(dmissRNID/AU) AU, $(dmissRNID*Msol2m/1000) km)")
end

println("Final target positions saved to Final_Prox_minus_Spacecraft.txt")
