...
function momentum_exchange_sweep()
    ...
    for nn in nn_values
        ...
        dx = Double64(1e14)*b  # Optimal separation (not too large, not too small)
        ...
        δ_initial = BigFloat(1000.0)          # Initial timestep (reduced for higher accuracy)
        courant = BigFloat(0.005)          # Courant number (reduced further for maximum accuracy)
